...
.PHONY: deploy-dev
deploy-dev:
	cd build && sudo vercel deploy --confirm --prod --token=$(ZEIT_TOKEN) --no-clipboard --local-config=.vercel/vercel.dev.json

.PHONY: deploy-prod
deploy-prod:
	cd build && sudo vercel deploy --confirm --prod --token=$(ZEIT_TOKEN) --no-clipboard --local-config=.vercel/vercel.prod.json
